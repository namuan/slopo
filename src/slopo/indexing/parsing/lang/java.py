...
_UNIT_TYPES = {"method_declaration", "lambda_expression"}
...
def _unit_name(node: Node) -> str:
    # Methods carry their own name. Lambdas are anonymous, so the name comes from
    # what they are bound to (a lambda passed as a call argument has no name and
    # stays <unknown>).
